...
impl<'a> SyncmerIterator<'a> {
    pub fn new(seq: &'a [u8], k: usize, s: usize, t: usize) -> SyncmerIterator<'a> {
        SyncmerIterator {
            ...
            s,
            t,
            // Overflow-safe: at the documented max k=32 (and s=32), `2*k`/`2*s` is
            // 64, and `1u64 << 64` is undefined (debug panic / release wrap to
            // `<<0`, giving a degenerate mask of 0). Use the full mask in that case.
            kmask: if 2 * k >= 64 { u64::MAX } else { (1u64 << (2 * k)) - 1 },
            smask: if 2 * s >= 64 { u64::MAX } else { (1u64 << (2 * s)) - 1 },
            kshift: (k - 1) * 2,
            sshift: (s - 1) * 2,
            qs: VecDeque::new(),
            qs_min_val: u64::MAX,
