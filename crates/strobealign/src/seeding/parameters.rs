...
/// Map a read length to the canonical length of its seeding profile.
///
/// Every read length within a profile produces identical seeding parameters
/// (see `test_seeding_parameters_similar_read_length`), so the canonical length
/// is what should key a cached index: 149/150/151 all map to 150, 95 maps to 100.
pub fn canonical_read_length(read_length: usize) -> usize {
    Profile::from(read_length)
        .length()
        .expect("read-length profiles always have a canonical length")
}
...
impl SeedingParameters {
    ...
    pub fn with_aux_len(mut self, aux_len: u8) -> Result<Self, InvalidSeedingParameter> {
        // The mask shift is `9 + aux_len`, so aux_len must keep it < 64. The old
        // bound (aux_len > 63) allowed aux_len 55..=63, where `9 + aux_len` is
        // 64..=72 — a u64 shift ≥ 64 (panic in debug, wrap to a wrong mask in
        // release). 54 is the true maximum (9 + 54 = 63).
        if aux_len > 54 {
            return Err(InvalidSeedingParameter::InvalidParameter(
                "aux length must be at most 54 (9 + aux_len must be < 64)",
            ));
        }
...
mod test {
    ...
    #[test]
    fn test_canonical_read_length_buckets() {
        assert_eq!(canonical_read_length(143), 150);
        assert_eq!(canonical_read_length(150), 150);
        assert_eq!(canonical_read_length(148), 150);
        assert_eq!(canonical_read_length(100), 100);
        assert_eq!(canonical_read_length(95), 100);
        assert_eq!(canonical_read_length(250), 250);
        assert_eq!(canonical_read_length(50), 50);
    }
