...
impl RandstrobeParameters {
    ...
    pub fn with_window(
        ...
        // Validate the RESOLVED values, not the raw Options. Comparing the Options
        // directly was wrong: Option's Ord makes None < Some(_), so e.g.
        // `with_window(Some(10), None)` compared `Some(10) > None` = true and
        // falsely rejected (even when self.w_max is a valid larger value), while
        // `with_window(None, Some(5))` on self.w_min=10 compared `None > Some(5)` =
        // false and accepted the invalid w_min=10 > w_max=5 state.
