...
pub fn make_index<'a>(
    ...
    // `RefRandstrobe` stores the strobe `position` and `ref_index` as `u32`
    // (see index.rs). The old `BucketIndex::MAX` guard above is effectively dead
    // (`BucketIndex = u64`), so guard the values that actually truncate: a single
    // contig longer than u32::MAX (~4.29 Gbp) or more than u32::MAX references
    // would silently wrap the packed position/ref_index and corrupt the index
    // (mass map failures). Human chromosomes (≤ ~249 Mbp) are far under this; the
    // limit only bites on pathological single-contig references.
    if let Some((i, r)) = references
        .iter()
        .enumerate()
        .find(|(_, r)| r.sequence.len() > u32::MAX as usize)
    {
        panic!(
            "Reference contig {i} is {} bp, which exceeds the {} bp strobe-position limit \
             (u32); split it into smaller contigs",
            r.sequence.len(),
            u32::MAX
        );
    }
    if references.len() > u32::MAX as usize {
        panic!(
            "{} references exceed the u32 ref-index limit ({})",
            references.len(),
            u32::MAX
        );
    }
