...
/// Lossless, order-preserving pack of an anchor into a single u128 sort key:
/// `ref_id` in bits 96..128, `ref_start` in bits 32..96, `query_start` in bits
/// 0..32. `ref_id` (`ref_index: u32`) and `query_start` (a read offset) are
/// u32-bounded; `ref_start` (`RefRandstrobe::position: u32`, plus a <256 offset)
/// fits the 64-bit middle field. Comparing the packed u128 is identical to
/// lexicographic (ref_id, ref_start, query_start), i.e. `Anchor`'s derived `Ord`.
#[inline]
fn pack_anchor(a: &Anchor) -> u128 {
    ((a.ref_id as u128) << 96) | ((a.ref_start as u128) << 32) | (a.query_start as u128)
}

#[inline]
fn unpack_anchor(k: u128) -> Anchor {
    Anchor {
        ref_id: (k >> 96) as usize,
        ref_start: ((k >> 32) & 0xFFFF_FFFF_FFFF_FFFF) as usize,
        query_start: (k & 0xFFFF_FFFF) as usize,
    }
}
...
impl Chainer {
    ...
    pub fn get_chains(
        ...
        for &is_revcomp in &orientations {
            ...
            // Sort on a packed key MATERIALIZED ONCE per anchor, not recomputed
            // per comparison. `pack_anchor` is an order-preserving lossless
            // bijection of (ref_id, ref_start, query_start) into a u128 (ref_id in
            // bits 96..128, ref_start in 32..96, query_start in 0..32 — ref_id and
            // query_start are u32-bounded by the index, ref_start is u32-bounded by
            // `RefRandstrobe::position: u32`). Sorting a `Vec<u128>` does one
            // 16-byte-POD compare per step with the key already computed, vs
            // `sort_unstable_by_key`'s ~n·log n key recomputations (profiled at
            // ~12% of mapper self-time) or `sort_unstable()`'s branchy 24-byte
            // struct swaps. dedup on the bijective key == dedup on the 3 fields, so
            // unpacking yields byte-identical mapping output.
            let mut keys: Vec<u128> = anchors.iter().map(pack_anchor).collect();
            keys.sort_unstable();
            keys.dedup();
            anchors.clear();
            anchors.extend(keys.iter().map(|&k| unpack_anchor(k)));
...
impl ChainingResult {
    fn extract_chains(&self, k: usize, is_revcomp: bool, chains: &mut Vec<Nam>) {
        ...
        for (i, score) in candidates {
            ...
            // Only the anchor COUNT is needed (for scoring + diagnostics); the
            // anchors themselves are never read after chaining, so track a counter
            // instead of allocating a per-chain Vec. Starts at 1 for anchor `i`.
            let mut num_anchors = 1usize;
            ...
            chains.push(Nam {
                ...
                matching_bases,
                ref_id: last.ref_id,
                score: score + num_anchors as f32 * self.parameters.matches_weight,
                is_revcomp,
                num_anchors,
            });
        }
