...
impl<'a> IndexEntry<'a> {
    ...
    pub fn is_too_frequent_forward(&self, cutoff: usize) -> bool {
        // Bound-check with `cutoff` — the value actually used to index below — not
        // `filter_cutoff`. If `cutoff > filter_cutoff`, checking `filter_cutoff` could pass
        // while `randstrobes[position + cutoff]` reads out of bounds (matches the
        // `_partial` variant's check).
...
pub fn read_index<'a, P: AsRef<Path>>(
    ...
    // Total file size is an upper bound on every length field we read below: no
    // vector or reserved chunk can contain more bytes than the file itself. Used
    // to reject corrupt/foreign (e.g. wrong-endian) length fields before they
    // drive a huge allocation (which would abort the process, not return Err).
    let file_size = file
        .metadata()
    ...
    let reserved_chunk_size = u64::from_ne_bytes(buf) as usize;
    if reserved_chunk_size > file_size {
        return Err(IndexReadingError::ParameterMismatch);
    }
    let mut buf = vec![0; reserved_chunk_size];
    reader.read_exact(&mut buf)?;
    ...
    // A corrupt or foreign-endian `.sti` can carry any 64-bit mask. `trailing_zeros() - 1`
    // underflows when the LSB is set (tz == 0), and the all-zero mask (tz == 64) is
    // degenerate; both then feed an out-of-range `1 << pos` shift. Reject before deriving
    // the orientation bit rather than panic (debug/overflow-checks) or compute a bogus mask.
    let tz = main_hash_mask.trailing_zeros();
    if tz == 0 || tz >= 64 {
        return Err(IndexReadingError::ParameterMismatch);
    }
    let partial_orientation_pos = tz - 1;
    ...
    let randstrobes = read_vec(&mut reader, file_size)?;
    let bucket_starts = read_vec(&mut reader, file_size)?;

    if bucket_starts.len() != (1 << bits) + 1 {
        return Err(IndexReadingError::RandstrobeStartIndicesWrongSize);
    }

    // Validate the VALUES, not just the length: bucket_starts is read verbatim
    // (POD) from the on-disk sidecar, and lookup later does
    // `&randstrobes[bucket_starts[h]..bucket_starts[h+1]]`. A corrupt/foreign-endian
    // sidecar with non-monotonic or out-of-range entries would otherwise panic with
    // an out-of-bounds slice DURING mapping — after read_index returned Ok, so the
    // Err→rebuild fallback in load_or_build_index can't catch it. Reject here so a
    // bad sidecar triggers a clean rebuild instead of a mapping-time crash.
    let n = randstrobes.len() as u64;
    if *bucket_starts.last().unwrap() != n
        || bucket_starts.windows(2).any(|w| w[0] > w[1])
        || bucket_starts.iter().any(|&s| s > n)
    {
        return Err(IndexReadingError::RandstrobeStartIndicesWrongSize);
    }
...
/// Validate ONLY the fixed header of a `.sti` file (magic, version, seeding
/// parameters) WITHOUT reading the large randstrobe/bucket vectors — cheap enough
/// for a preflight (reads a few hundred bytes). `bits`/`filter_cutoff` are NOT
/// checked here. Returns `Ok(())` iff the header is well-formed and the stored
/// seeding parameters equal `parameters`.
pub fn read_index_header<P: AsRef<Path>>(
    path: P,
    parameters: &SeedingParameters,
) -> Result<(), IndexReadingError> {
    let file = File::open(path)?;
    let file_size = file.metadata().map(|m| m.len() as usize).unwrap_or(usize::MAX);
    let mut reader = BufReader::new(file);
    ...
    // Skip the reserved chunk (bounded by file size so a corrupt length can't OOM).
    let mut buf = [0; 8];
    reader.read_exact(&mut buf)?;
    let reserved_chunk_size = u64::from_ne_bytes(buf) as usize;
    if reserved_chunk_size > file_size {
    ...
    let _filter_cutoff = read_u32(&mut reader)?;
    let _bits = read_u32(&mut reader)?; // intentionally not validated here
    let profile = read_u32(&mut reader)?
        .try_into()
        .map_err(|_| IndexReadingError::WrongProfile)?;
    let k = read_u32(&mut reader)? as usize;
    ...
    let syncmer = SyncmerParameters::try_new(k, s)?;
    // A corrupt or foreign-endian `.sti` can carry any 64-bit mask. `trailing_zeros() - 1`
    // underflows when the LSB is set (tz == 0), and the all-zero mask (tz == 64) is
    // degenerate; both then feed an out-of-range `1 << pos` shift. Reject before deriving
    // the orientation bit rather than panic (debug/overflow-checks) or compute a bogus mask.
    ...
    let partial_orientation_pos = tz - 1;
    let randstrobe = RandstrobeParameters {
        w_min,
        w_max,
        q,
        max_dist,
    ...
    let sti_parameters = SeedingParameters { profile, syncmer, randstrobe };
    if *parameters != sti_parameters {
        return Err(IndexReadingError::ParameterMismatch);
    }
    Ok(())
...
/// Read a length-prefixed `Vec<T>` written by [`write_vec`].
///
/// `T` must be plain-old-data: every byte pattern of `size_of::<T>()` bytes must
/// be a valid `T` with no padding (true for `RefRandstrobe` — `#[repr(C)]` of
/// `u64,u32,u32` — and for `u64`/`BucketIndex`). The bytes are read directly
/// into a `Vec<T>` allocation, which is correctly aligned for `T`; the previous
/// implementation reinterpreted a `Vec<u8>` (alignment 1) as `Vec<T>`, which is
/// UB on the allocator's dealloc layout.
///
/// `max_bytes` bounds the element-count field (an untrusted `u64` from the file)
/// so a corrupt or foreign-endian length can't drive a giant allocation that
/// would abort the process; it returns `Err` instead, triggering a rebuild.
fn read_vec<T, R: Read>(file: &mut BufReader<R>, max_bytes: usize) -> Result<Vec<T>, Error> {
    ...
    let byte_len = length
        ...
        .ok_or_else(|| {
            Error::new(
                ...
                "index vector length exceeds file size (corrupt or wrong-endian .sti)",
            )
        })?;

    let mut data: Vec<T> = Vec::with_capacity(length);
    // SAFETY: `data` owns an allocation aligned for `T` with capacity for
    // `length` elements. We read exactly `length * size_of::<T>()` bytes into
    // it as raw bytes, then `set_len(length)`. Sound because `T` is POD (any
    // bit pattern is a valid `T`, no padding), per this function's contract.
    unsafe {
        let dst = std::slice::from_raw_parts_mut(data.as_mut_ptr() as *mut u8, byte_len);
        file.read_exact(dst)?;
        data.set_len(length);
    }
...
mod tests {
    ...
    #[test]
    fn read_index_header_rejects_garbage() {
        let d = tempfile::tempdir().unwrap();
        let p = d.path().join("bad.sti");
        std::fs::write(&p, b"not an STI file at all").unwrap();
        let params = crate::seeding::SeedingParameters::new(150);
        let err = read_index_header(&p, &params).unwrap_err();
        assert!(matches!(err, IndexReadingError::WrongMagic), "got {err:?}");
    }
