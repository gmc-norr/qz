...
pub struct Nam {
    ...
    pub score: f32,
    pub is_revcomp: bool,
    /// Number of anchors in this chain. Used for scoring and diagnostics. The
    /// anchors themselves are not retained: nothing downstream of chaining reads
    /// them, so storing only the count avoids a per-NAM heap allocation (and a
    /// Vec clone on every NAM clone during pairing).
...
pub fn sort_nams(nams: &mut [Nam], rng: &mut Rng) -> f64 {
    ...
    if log::log_enabled!(Trace) {
        ...
        for nam in nams.iter() {
            if nam.num_anchors > 1 || printed < 10 {
                trace!("- {}", nam);
                printed += 1;
            }
        }
