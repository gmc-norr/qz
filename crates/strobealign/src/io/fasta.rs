...
pub fn read_ref<P: AsRef<Path>>(path: P) -> Result<Vec<RefSequence>, SequenceIOError> {
    // Propagate open failures (e.g. a missing --reference path) as an error
    // instead of panicking; `SequenceIOError: From<io::Error>`.
...
mod tests {
    ...
    #[test]
    fn read_ref_missing_file_errors_not_panics() {
        // A missing path must return an error, not panic (was File::open(_).unwrap()).
        let r = read_ref("/nonexistent/strobealign-missing-reference.fasta");
        assert!(r.is_err());
    }
